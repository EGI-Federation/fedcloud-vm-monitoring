...
from utils import colourise, highlight, get_settings, pretty_hostname, check_SSL_certificate
    ...
    with open (env['PROVIDERS_SETTINGS_FILENAME'], 'w') as file:
         ...
         for item in endpoints.findall('SERVICE_ENDPOINT'):
             ...
             if 'Y' in production and 'Y' in monitored:
                print("\n- Fetching metadata from the resource provider: %s" %colourise("cyan",sitename))
                ...
                token = get_OIDC_Token(settings['checkin_auth_url'],
                ...
                try:
                    ...
                    print(colourise("green", "- No.%d tenant(s) supported by the resource provider" \
                          %len(projects)))
                    ...
                    for project in projects:
                        ...
                        if (env['TENANT_NAME'] in project_name):
                           print(colourise("yellow", "- Tenant published by the resource provider"))
                           # Retrieve a "scoped" token from the provider
                           scoped_token = get_scoped_Token(os_auth_url, project_id, unscoped_token)
                           # Get the list of catalogue services
                           endpoints = get_service_catalogue_details(os_auth_url, scoped_token)
                           ...
                           for endpoint in endpoints:
                               ...
                               for url in urls:
                                   ...
                                   if "public" in endpoint_interface and \
                                      "nova" in endpoint_name and \
                                      "nova-placement" not in endpoint_name:
                                      #print(endpoint_name, endpoint_url, endpoint_interface)
